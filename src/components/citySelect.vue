<template>
  <div>
    ...
    <div class="mask" v-if="show" @click.native="close" ref="mask" :opacity="maskerOpacity"></div>
    <div class="yd-cityselect" :class="show ? 'yd-cityselect-active' : ''">
      <div class="yd-cityselect-header">
        ...
        <div v-show="ready" class="yd-cityselect-nav">
          <a href="javascript:;" :key="key" v-for="(index, key) in columnNum" v-show="!!nav['txt' + index]" @click.stop="navEvent(index)" :class="index == navIndex ? 'yd-cityselect-nav-active' : ''">{{nav['txt' + index]}}</a>
        </div>
      </div>
        ...
        <li class="yd-cityselect-item" v-for="(index, key) in columnNum" :ref="'itemBox' + index" :key="key">
          <template v-if="columnsObj['columnItems' + index] && columnsObj['columnItems' + index].length > 0">
            <div class="yd-cityselect-item-box">
              <a href="javascript:;" :key="key" v-for="(item, key) in columnsObj['columnItems' + index]" :class="currentClass(item.v, item.n, index)" @click.stop="itemEvent(index, item.n, item.v, item.c)">
                <span>{{item.n}}</span>
              </a>
            </div>
          </template>
...
// import { isIOS, pageScroll } from '../JS/assist'
...
export default {
  ...
  methods: {
    ...
    setDefalutValue (items, currentValue, index) {
      items.every((item, key) => {
        if (item.v === this[currentValue] || item.n === this[currentValue]) {
          const childrenItems = (this.columnsObj['columnItems' + (index + 1)] =
            item.c)
    ...
    backoffView (animate) {
      this.activeClasses =
        (animate ? 'yd-cityselect-move-animate' : '') + ' yd-cityselect-prev'
    },
    forwardView (animate) {
      this.activeClasses =
        (animate ? 'yd-cityselect-move-animate' : '') + ' yd-cityselect-next'
    }
  },
  mounted () {
    this.init()
