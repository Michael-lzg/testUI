<template>
  <div class="yd-progressbar">
    <div class="yd-progressbar-content" v-if="type != 'line'">
      <slot></slot>
    </div>
    <svg :viewBox="viewBox" v-show="show" :preserveAspectRatio="type == 'line' ? 'none' : ''">
      <path :fill-opacity="!!fillColor ? 1 : 0" :d="getPathString" :fill="fillColor" :stroke="strokeColor" :stroke-width="trailWidth"></path>
      <path fill-opacity="0" ref="trailPath" :d="getPathString" :stroke="trailColor" :stroke-width="strokeWidth ? strokeWidth : trailWidth" :style="{strokeDasharray: stroke.dasharray, strokeDashoffset: stroke.dashoffset}"></path>
    </svg>
  </div>
</template>

<script type="text/babel">
import { isColor, getScrollview, checkInview } from '../JS/assist'
...
export default {
  ...
  methods: {
    init () {
      ...
      if (this.type === 'line') {
        this.viewBox =
          '0 0 100 ' + (this.strokeWidth ? this.strokeWidth : this.trailWidth)
      }
  ...
  computed: {
    getPathString () {
      if (this.type === 'line') {
        ...
        const r =
          50 - (this.strokeWidth ? this.strokeWidth : this.trailWidth) / 2
